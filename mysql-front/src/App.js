...
function App() { 
  ...
  // useEffect 는 버튼이나 특정값을 클릭하지 않아도 자동실행 딱 1 번만 실행되느냐, 주기적으로 실행하느냐
  // App.js 가 실행되면 적용할 효과, 만약 특정변수명이 없다면 최초 1 회만 실행하고 실행되지않음
  // useEffect(() => {기능}, [         ]);
...
/*
  const 모든유저보기 = () => {
    // axios 를 이용해서 모든 유저를 보겠다
    axios.get("/users") // controller GetMapping 에서 /users 라는 주소를 바라보기 때문에 users 를 적어준 것

    // 응답을 무사히 가져왔을 때
    .then(response => { // Java 에서 DB 값에 있는 내용을 가져와 고객에게 가져온 내용에 대한 응답을 알려주는 것
      setUsers(response.data); // 응답 결과 데이터로 users 를 변경하겠다.
    })
    .catch(err => {
      alert("가져오지 못했습니다."); // 주로 alert 보다 console.log 로 작성해서 개발자가 에러를 볼 수 있게 함
    })
  }
  */
...
/* 
  ...
  // async = 기능 실행해보자  await = 기다리자 값 가져와볼게
  const 모든유저보기 = async () => {
...
const updateUser = async (user) => {
  await axios.put('/users', user); // putmapping 에서 /users 로 주소값이 설정된 수정하는 주소 연결
  setUsers(users.map(u => (u.id === user.id ? user : u)));
  // 수정한 유저의 id 값이 일치하는지 확인하고, id 값이 일치하지 않다면 기존에 있던 유저 정보로 수정하지 않고 전달 
}
...
/* 수정하기 버튼이 있다면 수정취소하기 확인 버튼도 */
const cancelEdit = () => {
  setUserToEdit(null); // 유저 정보 수정 취소할 때 null 값으로 변경하는 트릭
}
