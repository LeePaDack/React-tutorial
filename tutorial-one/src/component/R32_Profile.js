...
const Profile = () => {
    ...
    const 이미지변경하기 = (event) => {
        const file = event.target.files[0]; // 파일은 리스트 목록으로 존재 0 번째부터 ~~ 있겠다.
        // 추후 이미지 다중선택이 가능할 때 이미지 순서대로 가지고 올 것
        // 파일은 모두 파일들이라는 전제하에 파일을 선택하기 때문에
        // 하나의 이미지를 선택할 때는 첫번째 자리 (index = 0) 인 0 번 쨰 사진을 갖고오겠다.
        if(file) { // 만약에 이미지 파일이 존재한다면
            ...
            reader.onloadend = () => { // 파일 읽기가 완료되었을 때 실행될 함수를 표현
                ...
                // (이미지를 컴퓨터 용어로 작성된 코드를 주소값으로 변경한 데이터 주소(URL))
                set선택한이미지(reader.result);
            };
            reader.readAsDataURL(file); 
            // readAsDataURL(file) 기능 : 데이터 URL 을 글자취급해서 읽기 시작 
            // 주소는 String 문자열로 표현
        }
    }
    ...
    return (
        <div className="profile-container">
            ...
            <h1>프로필 사진 변경</h1>
            {/* input 태그숨기고 label 로 클릭해서 이미지 보여주기 */}
